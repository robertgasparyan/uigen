...
test("MessageList shows friendly label for file_manager tool", () => {
  const messages: Message[] = [{
    id: "1", role: "assistant", content: "",
    parts: [{
      type: "tool-invocation",
      toolInvocation: { toolCallId: "x", args: {}, toolName: "file_manager", state: "result", result: "ok" },
    }],
  }];
  render(<MessageList messages={messages} />);
...
test("MessageList falls back to raw tool name for unknown tools", () => {
  const messages: Message[] = [{
    id: "1", role: "assistant", content: "",
    parts: [{
      type: "tool-invocation",
      toolInvocation: { toolCallId: "x", args: {}, toolName: "unknown_tool", state: "result", result: "ok" },
    }],
  }];
  render(<MessageList messages={messages} />);
